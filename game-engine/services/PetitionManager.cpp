...
std::vector<CompletedConstruction> PetitionManager::tick()
{
    ...
    for (auto& petition : underConstructionPetitions) {
        ...
        if(!effects.empty()) {
            completedConstructions.push_back({ petition->getBuildingType(), std::move(effects) });
...
Petition* PetitionManager::acceptPetition()
{
    if (currentPetition != nullptr)
    {
        Petition* acceptedPetition = currentPetition;
        underConstructionPetitions.push_back(acceptedPetition);
        currentPetition = generatePetition();
